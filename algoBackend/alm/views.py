...
class BondPortofolioViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        user = request.user
        bilan = Bilan.objects.get(user=user, is_active=True)
...
class BondPortofolioViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        user = request.user
        bilan = get_object_or_404(Bilan, user=user, is_active=True)
        request.data["user"] = user.id
        request.data["bilan"] = bilan.id

        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        start_value = serializer.validated_data.get("start")

        bond_portofolio = serializer.save()

        if start_value == '0':
            bond_portofolio.is_active = True
            bond_portofolio.save()
        elif start_value == '1':
            active_portofolio = BondPortofolio.objects.filter(user=user, is_active=True).exclude(id=bond_portofolio.id).first()
            print("Active portofolio", "Before")
            if active_portofolio:
                print("Active portofolio", active_portofolio)
                bonds_to_duplicate = active_portofolio.bond.all()
                for bond in bonds_to_duplicate:
                    print("Bond", bond)
                    bond.pk = None 
                    bond.portofolio = bond_portofolio
                    bond.save()

            bond_portofolio.is_active = True
            bond_portofolio.save()

        # Ensure only one active portfolio per bilan
        BondPortofolio.objects.filter(user=user, bilan=bilan).exclude(id=bond_portofolio.id).update(is_active=False)
    ...
    def switch(self, request, pk=None):
        user = request.user
        portofolio = self.get_object()
        bilan = portofolio.bilan

        try:
            active_portofolio = BondPortofolio.objects.get(user=user, bilan=bilan, is_active=True)
            if active_portofolio.id == portofolio.id:
                return Response(
                    {"message": "The selected portfolio is already active."}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
            
            active_portofolio.is_active = False
            active_portofolio.save()
        except BondPortofolio.DoesNotExist:
            pass
        ...
        return Response(
            {"message": "Portfolio switched successfully"}, status=status.HTTP_200_OK
        )
...
class BondsViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # try:
        user = request.user
        admin_bond = AdminBond.objects.get(id=request.data['isin'])
        
        bilan = Bilan.objects.get(user=user, is_active=True)
        portofolio = BondPortofolio.objects.get(bilan=bilan, is_active=True)
