...
from .serializers import OPInjectionSerializer,OPInjectionCreateSerializer

import math
from datetime import datetime, timedelta
from dateutil.relativedelta import relativedelta

class OPInjectionViewSet(viewsets.ModelViewSet):
    queryset = OPInjection.objects.all()
    serializer_class = OPInjectionSerializer

    def get_serializer_class(self):
        return OPInjectionCreateSerializer if self.action == 'create' else OPInjectionSerializer
    
    def perform_create(self, serializer):
        data = serializer.validated_data

        outstanding = data['op_injection_outstanding']
        value_date = data['op_injection_value_date']
        due_date = data['op_injection_due_date']
        taux_interet = data['op_injection_rate']
        ...
        while date_courante < date_paiement:
            months_difference = (due_date - date_courante).months
            days_difference = (due_date - date_courante).days
            duration = max(0, months_difference)
        ...
        serializer.save(
            op_injection_interest=interest,
            op_injection_cashflows=cashflows
        )