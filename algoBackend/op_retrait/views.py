...
from .serializers import OPRetraitSerializer,OPRetraitCreateSerializer

import math
from datetime import datetime, timedelta
from dateutil.relativedelta import relativedelta

class OPRetraitViewset(viewsets.ModelViewSet):
    queryset = OPRetrait.objects.all()
    serializer_class = OPRetraitSerializer

    def get_serializer_class(self):
        return OPRetraitCreateSerializer if self.action == 'create' else OPRetraitSerializer
    
    def perform_create(self, serializer):
        data = serializer.validated_data

        outstanding = data['op_retrait_outstanding']
        value_date = data['op_retrait_value_date']
        due_date = data['op_retrait_due_date']
        taux_interet = data['op_retrait_rate']
        ...
        while date_courante < date_paiement:
            ...
            cashflows[date_courante.strftime('%Y-%m-%d')] = {
                ...
                'duration' :                            duration,
                'duration_times_outstanding' :          duration * outstanding,
        ...
        serializer.save(
            op_retrait_interest=interest,
            op_retrait_cashflows=cashflows
        )